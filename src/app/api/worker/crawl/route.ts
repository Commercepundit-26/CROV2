...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (process.env.QSTASH_TOKEN) {
      await qstashClient.publishJSON({ url: `${baseUrl}/api/worker/analyze`, body: { jobId } });
    } else {
      fetch(`${baseUrl}/api/worker/analyze`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ jobId }) }).catch(console.error);
    }
